#!/usr/bin/env python3
"""Apply a verified Photo Mode pause-toggle hook to AMS.exe.

This mirrors the Replay frame safety model but targets selector 0xC0DEB004.
...
def load_binding(path: Path) -> dict:
    ...
    if data.get("format") != "rextreme-photo-toggle-binding":
    ...
    if data.get("enabled") is not True:
        raise BindingError("no verified Photo Mode pause-toggle binding is enabled")
    b = data.get("binding")
...
def pic_gateway_call(cave_va: int, prefix_len: int) -> bytes:
    code = bytearray()
    code += b"\x68" + struct.pack("<I", PHOTO_TOGGLE_MAGIC)
...
def apply(ams: Path, binding_path: Path, project_root: Path, check_only: bool) -> dict:
    ...
    stub = build_stub(b)
    patch = site_patch(b)
    require(data, b["site_file_offset"], b["expected"], "Photo toggle hook site")
    require(data, b["cave_file_offset"], expected_cave(b), "Photo toggle cave")

    before = hashlib.sha256(data).hexdigest()
    report = {
        "format": "rextreme-photo-toggle-binding-apply",
        "version": 1,
        "build": "1.7.3.8-x86",
        "selector": f"0x{PHOTO_TOGGLE_MAGIC:08X}",
    ...
    backup_dir = project_root / "_BACKUPS" / "PHOTO-TOGGLE-BINDING"
    ...
    tmp = ams.with_suffix(".photo-toggle.tmp")
    tmp.write_bytes(data)
    verify = tmp.read_bytes()
    require(verify, b["site_file_offset"], patch, "Photo toggle redirect verification")
    require(verify, b["cave_file_offset"], stub, "Photo toggle stub verification")
    tmp.replace(ams)
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Apply verified ReXtreme Photo Mode pause-toggle AMS binding")
    ap.add_argument("ams", type=Path)
    ap.add_argument("--binding", type=Path, default=Path("config/photo-toggle-binding.verified.json"))
    ap.add_argument("--project-root", type=Path, default=Path("."))
    ap.add_argument("--check-only", action="store_true")
    ap.add_argument("--report", type=Path, default=Path("_TRACE_MONTAR/PHOTO-TOGGLE-BINDING.json"))
    ns = ap.parse_args()
