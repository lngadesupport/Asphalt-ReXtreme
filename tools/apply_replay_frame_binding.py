#!/usr/bin/env python3
"""Apply a verified Replay per-frame hook to AMS.exe.

Fail-closed by design:
- source JSON must target 1.7.3.8-x86;
- enabled must be true;
- binding.verified must be true;
- evidence must be non-empty;
- exact site/cave bytes must match;
- overwritten instructions may not contain relative control flow;
- original bytes are replayed before returning to the original method.

The generated stub forwards GameModeGUIBase* to selector 0xC0DEB002.
"""
...
def validate_pe32_x86(data: bytes) -> None:
    if len(data) < 0x100 or data[:2] != b"MZ":
        raise BindingError("AMS.exe is not an MZ executable")
    ...
    if struct.unpack_from("<I", data, opt + 28)[0] != IMAGE_BASE:
        raise BindingError("unexpected preferred image base")
...
def validate_relocatable_original(original: bytes, site_va: int) -> None:
    try:
        ...
        raise BindingError("capstone is required: py -3 -m pip install capstone") from exc

    md = Cs(CS_ARCH_X86, CS_MODE_32)
    consumed = 0
    instructions = list(md.disasm(original, site_va))
    for ins in instructions:
        consumed += ins.size
        mnemonic = ins.mnemonic.lower()
        if mnemonic.startswith("j") or mnemonic.startswith("loop") or mnemonic == "call":
            raise BindingError(
                f"site contains relative/control-flow instruction {mnemonic} at 0x{ins.address:08X}; "
                "generic relocation is not safe"
            )
...
def pic_gateway_call(cave_va: int, prefix_len: int) -> bytes:
    code = bytearray()
    code += b"\x68" + struct.pack("<I", REPLAY_FRAME_MAGIC)
...
def load_binding(path: Path) -> dict:
    ...
    if data.get("format") != "rextreme-replay-frame-binding":
        raise BindingError("invalid binding format")
    if data.get("version") != 1:
        raise BindingError("unsupported binding version")
    if data.get("build") != "1.7.3.8-x86":
        raise BindingError("binding must target 1.7.3.8-x86")
    if data.get("enabled") is not True:
        raise BindingError("no verified Replay frame binding is enabled")
    b = data.get("binding")
...
def normalize_binding(raw: dict) -> dict:
    ...
    if len(cave_fill) not in (1, cave_len):
        raise BindingError("cave_expected_hex must be one fill byte or exactly cave_length bytes")
    ...
    if resume_va != site_va + len(original):
        raise BindingError("resume_va must equal site_va + len(expected_hex)")
...
def build_stub(binding: dict) -> bytes:
    cave_va = binding["cave_va"]
    cave_len = binding["cave_length"]
    original = binding["expected"]
    this_register = binding["this_register"]

    code = bytearray()
    code += b"\x9C\x60"  # pushfd / pushad
    code += REG_TO_ECX[this_register]
    code += pic_gateway_call(cave_va, len(code))
    ...
    if len(code) > cave_len:
        raise BindingError(
            f"generated stub needs {len(code)} bytes but cave_length is {cave_len}"
        )
    return bytes(code) + b"\xCC" * (cave_len - len(code))


def site_patch(binding: dict) -> bytes:
    original = binding["expected"]
    patch = b"\xE9" + rel32(binding["site_va"], 5, binding["cave_va"])
    return patch + b"\x90" * (len(original) - len(patch))


def expected_cave(binding: dict) -> bytes:
    fill = binding["cave_fill"]
    if len(fill) == 1:
        return fill * binding["cave_length"]
    return fill
...
def apply(ams: Path, binding_path: Path, project_root: Path, check_only: bool) -> dict:
    ...
    b = normalize_binding(raw)
    data = bytearray(ams.read_bytes())
    validate_pe32_x86(data)

    stub = build_stub(b)
    patch = site_patch(b)
    cave_expected = expected_cave(b)

    require(data, b["site_file_offset"], b["expected"], "Replay frame hook site")
    require(data, b["cave_file_offset"], cave_expected, "Replay frame code cave")

    before = sha256_bytes(data)
    report = {
        "format": "rextreme-replay-frame-binding-apply",
        "version": 1,
        "build": "1.7.3.8-x86",
        "check_only": check_only,
        "selector": f"0x{REPLAY_FRAME_MAGIC:08X}",
        "site_va": f"0x{b['site_va']:08X}",
        "site_file_offset": f"0x{b['site_file_offset']:08X}",
        "resume_va": f"0x{b['resume_va']:08X}",
        "cave_va": f"0x{b['cave_va']:08X}",
        "cave_file_offset": f"0x{b['cave_file_offset']:08X}",
        "cave_length": b["cave_length"],
    ...
    backup_dir = project_root / "_BACKUPS" / "REPLAY-FRAME-BINDING"
    ...
    tmp = ams.with_suffix(".replay-frame.tmp")
    tmp.write_bytes(data)
    verify = tmp.read_bytes()
    require(verify, b["site_file_offset"], patch, "Replay frame redirect verification")
    require(verify, b["cave_file_offset"], stub, "Replay frame stub verification")
    tmp.replace(ams)
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Apply verified ReXtreme Replay per-frame AMS binding")
    ap.add_argument("ams", type=Path)
    ap.add_argument(
        "--binding",
        type=Path,
        default=Path("config/replay-frame-binding.verified.json"),
    )
    ap.add_argument("--project-root", type=Path, default=Path("."))
    ap.add_argument("--check-only", action="store_true")
    ap.add_argument("--report", type=Path, default=Path("_TRACE_MONTAR/REPLAY-FRAME-BINDING.json"))
    ns = ap.parse_args()
    ...
    if report["applied"]:
        print("[OK] verified Replay frame hook applied")
    else:
        print("[OK] verified Replay frame hook bytes validated; no patch written")
    return 0
