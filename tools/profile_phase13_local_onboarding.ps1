...
if($Restore){
    if(-not (Test-Path -LiteralPath $Backup -PathType Leaf)){
        throw "Phase 13 backup not found: $Backup"
    }
    if((Get-Hash $Backup) -ne $ExpectedPhase5){
        throw "Phase 13 backup is not verified Phase 5."
    }
    Copy-Item -LiteralPath $Backup -Destination $Ams -Force
    Write-Host ""
    Write-Host "PHASE 13 RESTORED TO VERIFIED PHASE 5" -ForegroundColor Green
    exit 0
}
...
$patches=@(
    ...
    # ------------------------------------------------------------------
    # Phase 13: repair the age/gender handler itself.
    #
...
$status=[ordered]@{
    ...
    Notes=@(
        ...
        "Phase 12 body skips at 0x006A1CE7 and 0x006A1E09 are intentionally removed because they prematurely terminate the onboarding handler.",
        "No remote profile, Store or advertising service is re-enabled.",
        "The first runtime check is whether age/gender ACEITAR now advances without a network popup or freeze."
    )
}
...
Write-Host ""
Write-Host "============================================================"
Write-Host " PHASE 13 - LOCAL ONBOARDING HANDLER APPLIED" -ForegroundColor Green
Write-Host "============================================================"
Write-Host "Logical connectivity remains offline: True"
Write-Host "Age/gender local-success gates patched: 2 / 2"
Write-Host "Former Phase 12 age-handler body skips: removed / superseded"
Write-Host "Other known NO_INTERNET paths retained: 13"
Write-Host ("Total binary patches: {0}" -f $patches.Count)
Write-Host ("Patched AMS SHA-256: {0}" -f $status.PatchedAMS_SHA256)
Write-Host ""
Write-Host "First runtime check:"
Write-Host "  age/gender -> choose age 21 -> ACEITAR"
Write-Host "Expected:"
Write-Host "  handler completes locally; no FALHA DE CONEXAO; startup advances."
Write-Host ""
