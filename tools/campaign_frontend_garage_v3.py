...
def pic_gateway_stub(stub_va:int,selector:int)->bytes:
    ...
    code += b"\xC3"
    if len(code)!=19:
        raise RuntimeError(f"unexpected stub size {len(code)}")
    return bytes(code)

BUILD_PATCH=pic_gateway_stub(BUILD_VA,BUILD_MAGIC)
GBBW_BUILD_PATCH=pic_gateway_stub(GBBW_BUILD_CALLBACK_VA,GBBW_BUILD_MAGIC)
...
def apply(root:Path)->int:
    ...
    patch_entry(
        ...
        "GarageBottomBarWidget MONTAR callback -> local Campaign build",changes
    )
    patch_entry(
    ...
    before=sha(raw)
    backup_dir=root/"_BACKUPS"/"FRONTEND-ONLY-GARAGE-V3"
    backup_dir.mkdir(parents=True,exist_ok=True)
    backup=backup_dir/f"AMS.exe.{before}.bak"
    if not backup.exists():
        shutil.copy2(ams,backup)

    tmp=ams.with_suffix(".frontend-garage-v3.tmp")
    tmp.write_bytes(d)
    ...
    report={
      "phase":"Frontend-Only Garage v3",
      "architecture":"direct local button callback",
      "network_authority":False,
      "multiplayer":False,
      "legacy_async_build_signal":"bypassed at GBBW build callback entry",
      "gbbw_build_callback":{
        "preferred_va":f"0x{GBBW_BUILD_CALLBACK_VA:08X}",
        "file_offset":f"0x{gbbw_cb_off:08X}",
        "selector":f"0x{GBBW_BUILD_MAGIC:08X}",
        "route":"GBBW owner pair -> validated GS_Garage -> CampaignGarageService"
      },
      ...
      "retired_from_montar":[
        "GBBW build signal emission",
        "CraftCarCaller",
        "CraftCarRequestImpl",
        "craft_car.php",
        "remote completion",
        "server callback",
        "GlobalSync build authority"
      ],
    ...
    rp=out/"FRONTEND-ONLY-GARAGE-V3.json"
    rp.write_text(json.dumps(report,indent=2,ensure_ascii=False),encoding="utf-8")

    print("[OK] Frontend-Only Garage v3 applied.")
    print("[MONTAR] button callback -> direct local Campaign build")
    print("[SIGNAL] legacy async GBBW build signal -> BYPASSED")
    print("[BUILD] GS_Garage fallback -> local Campaign build")
    print("[NETWORK] no request / no server / no multiplayer")
