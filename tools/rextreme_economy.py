...
def premium_price(original: int, multiplier: float = 0.80) -> int:
    # User requirement: true 20% reduction, rounded only to a whole credit.
...
def parse_shop(path: Path) -> dict[int, dict]:
    ...
    root = ET.fromstring(
        first_xml_document(text, "AsphaltShopConfiguration")
    )
    ...
    for car in root.findall("./Car"):
        ...
        if prokit is not None:
            for price in prokit.findall("./Price"):
                ...
                if currency in upgrades:
                    upgrades[currency].append(
                        int(float(price.get("Price", "0")))
                    )
        ...
        result[car_id] = {
            ...
            "upgrade_hardcurrency_total": sum(
                upgrades["hardcurrency"]
            ),
        }
...
def analyze(
    ...
    for car_id in sorted(shop):
        ...
        if credits > 0 and hardcurrency > 0:
            pricing_mode = "dual"
            dual_ratios[definition.get("class", "?")].append(
                credits / hardcurrency
            )
        elif credits > 0:
        ...
        vehicle_rows.append({
            ...
            "rextreme_credits_80pct": (
                premium_price(credits, multiplier)
                if credits > 0 else ""
            ),
            "token_only_credit_conversion": (
                "TBD"
                if credits == 0 and hardcurrency > 0 else ""
            ),
            "upgrade_credit_total_original":
                shop_car["upgrade_credit_total"],
            "upgrade_hardcurrency_total_original":
                shop_car["upgrade_hardcurrency_total"],
        })

    write_csv(
        output / "cars.csv",
        vehicle_rows,
        list(vehicle_rows[0].keys()),
    )

    seasons = {
        int(item.get("seasonid", item.get("id"))): item
        for item in career["seasons"]
    }

    event_rows = []
    season_aggregation = defaultdict(
        lambda: {"events": 0, "repeat_first": [], "star_credits": 0}
    )
    ...
    for event in career["events"]:
        ...
        for prefix in ("first", "second", "third"):
            if (
                str(event.get(
                    f"{prefix}_star_reward_type", ""
                )).lower()
                == "credits"
            ):
                star_credits += int(
                    event.get(
                        f"{prefix}_star_reward_amount", 0
                    ) or 0
                )
        ...
        row = {
            ...
            "completion_reward_type":
                event.get("completion_reward_type", ""),
            "completion_reward_amount":
                event.get("completion_reward_amount", 0),
    ...
    for season_id in sorted(seasons):
        ...
        for reward in season.get("rewards", []):
            reward_type = str(
                reward.get("completionrewardtype", "")
            ).lower()
            amount = int(
                reward.get("completionrewardamount", 0) or 0
            )
        ...
        season_rows.append({
            ...
            "repeat_first_place_avg": (
                round(sum(repeat_values) / len(repeat_values), 2)
                if repeat_values else 0
            ),
            "repeat_first_place_min": (
                min(repeat_values) if repeat_values else 0
            ),
            "repeat_first_place_max": (
                max(repeat_values) if repeat_values else 0
            ),
            "one_time_star_credits_total":
                aggregate["star_credits"],
            "season_completion_credits": completion_credits,
            "season_completion_hardcurrency":
                completion_hardcurrency,
    ...
    summary = {
        ...
        "vehicle_credit_multiplier": multiplier,
        "career_event_count": len(event_rows),
        "season_count": len(season_rows),
        "dual_price_credit_per_token_by_class": ratio_summary,
        "dual_price_credit_per_token_overall_median": (
            round(statistics.median(all_ratios), 2)
            if all_ratios else None
        ),
        "note": (
            "Token-only vehicle conversion is deliberately left TBD. "
            "The original dual-price ratios vary too much for one "
            "global token-to-credit exchange rate."
        ),
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Analyze Asphalt ReXtreme decrypted shop/career data"
        )
    )
    ...
    if not 0 < args.vehicle_multiplier <= 1:
        raise SystemExit(
            "vehicle multiplier must be greater than 0 and <= 1"
        )
