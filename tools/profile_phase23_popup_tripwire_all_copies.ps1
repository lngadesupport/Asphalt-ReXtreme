...
# Trap both known copies of each NO_INTERNET localization key.
$targets=@(
  [pscustomobject]@{VA=[uint32]0x015360F0;Kind="TITLE-A"},
  [pscustomobject]@{VA=[uint32]0x015360CC;Kind="DESCRIPTION-A"},
  [pscustomobject]@{VA=[uint32]0x01558F4D;Kind="TITLE-B"},
  [pscustomobject]@{VA=[uint32]0x01558F29;Kind="DESCRIPTION-B"}
)
$traps=New-Object System.Collections.Generic.List[object]
foreach($target in $targets){
  foreach($x in @(FindPtrPushes $target.VA $target.Kind)){
...
[ordered]@{
  Phase="23-popup-tripwire-all-copies"
...
Write-Host ""
Write-Host "============================================================"
Write-Host " PHASE 22 NO_INTERNET ALL-COPY TRIPWIRES INSTALLED" -ForegroundColor Green
Write-Host "============================================================"
