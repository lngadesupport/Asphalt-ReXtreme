#!/usr/bin/env python3
"""Create Campaign shop data: every positive direct car price is 20% of original."""
...
def main() -> int:
    ...
    if not 0 < a.vehicle_multiplier <= 1:
        raise SystemExit("vehicle multiplier must be > 0 and <= 1")

    text = a.asphaltshop.read_text(encoding="utf-8-sig")
    changes = []
    ...
    print(json.dumps({k: report[k] for k in (
        "changed_entries", "cars_changed", "credits_changes", "hardcurrency_changes", "output_bytes"
    )}, indent=2))
    return 0
