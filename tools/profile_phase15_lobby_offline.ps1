...
$BackupRoot=Join-Path $GameRoot "_PROFILE_PHASE15_BACKUP"
$Backup=Join-Path $BackupRoot "AMS.phase5.bak"
$ReportPath=Join-Path $GameRoot "PROFILE-PHASE15-LOBBY-OFFLINE-REPORT.json"
...
if($Restore){
    if(-not (Test-Path -LiteralPath $Backup -PathType Leaf)){
        throw "Phase 15 backup not found: $Backup"
    }
    if((Get-Hash $Backup) -ne $ExpectedPhase5){
        throw "Phase 15 backup is not verified Phase 5."
    }
    Copy-Item -LiteralPath $Backup -Destination $Ams -Force
    Write-Host ""
    Write-Host "PHASE 15 RESTORED TO VERIFIED PHASE 5" -ForegroundColor Green
    exit 0
}
...
$patches=@(
    ...
    # ------------------------------------------------------------------
    # Phase 15: repair the age/gender handler itself.
    #
    ...
    # ------------------------------------------------------------------
    # Phase 15: uncovered STR_POPUP_NO_INTERNET_TITLE paths.
    #
    # The Phase 15 map found ten title-only xrefs in addition to the
    ...
    # ------------------------------------------------------------------
    # Phase 15: dispatcher-specific lobby offline branch.
    #
    # Function 0x00916AE0 classifies status/error values in ESI.  Two
    # values (0x0BC2 and 0x0FAA) branch to 0x0091712C, the block that
    # builds STR_POPUP_NO_INTERNET_TITLE and its retry modal.
    #
    # Existing handled branches in this same function converge at
    # 0x00917409.  Redirect only these two no-internet decisions there.
    # ------------------------------------------------------------------
    [pscustomobject]@{
        Name="Lobby dispatcher error 0x0BC2 -> common epilogue"
        Offset=[Convert]::ToInt32("00916B30",16)
        Before=[byte[]](0x0F,0x84,0xF6,0x05,0x00,0x00)
        After=[byte[]](0x0F,0x84,0xD3,0x08,0x00,0x00)
    },
    [pscustomobject]@{
        Name="Lobby dispatcher error 0x0FAA -> common epilogue"
        Offset=[Convert]::ToInt32("00916B3C",16)
        Before=[byte[]](0x0F,0x84,0xEA,0x05,0x00,0x00)
...
$status=[ordered]@{
    Phase="15-lobby-offline-dispatcher"
    ...
    LobbyDispatcherNoInternetCodesBypassed=2
    ...
    Notes=@(
        ...
        "All ten additional STR_POPUP_NO_INTERNET_TITLE paths found by the Phase 15 map are bypassed.",
        ...
        "Dispatcher 0x00916AE0 codes 0x0BC2 and 0x0FAA now bypass the 0x0091712C no-internet modal block and converge at 0x00917409.",
        "Runtime acceptance test: saved tutorial state remains intact and lobby opens without SEM CONEXAO / NOVAMENTE."
    )
}
...
Write-Host ""
Write-Host "============================================================"
Write-Host " PHASE 15 - LOBBY OFFLINE DISPATCH APPLIED" -ForegroundColor Green
Write-Host "============================================================"
...
Write-Host "Lobby dispatcher NO_INTERNET branches bypassed: 2 / 2"
...
Write-Host "  launch saved profile -> direct lobby -> normal local navigation"
Write-Host "Expected:"
