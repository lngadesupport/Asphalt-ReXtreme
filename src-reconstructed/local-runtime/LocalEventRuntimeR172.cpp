...
constexpr bool kEnableOverlayUi = false; // R17.2: core first; overlay returns after runtime stability is proven.
...
bool ValidateAmsModule(BYTE** build_car_target) {
    if (!build_car_target) return false;
    *build_car_target = nullptr;

    HMODULE ams = GetModuleHandleW(L"AMS.exe");
    if (!ams) {
        Log(L"R17.2: AMS.exe module not found; hook disabled.");
        return false;
    }

    __try {
        BYTE* base = reinterpret_cast<BYTE*>(ams);
        auto* dos = reinterpret_cast<IMAGE_DOS_HEADER*>(base);
        if (dos->e_magic != IMAGE_DOS_SIGNATURE) {
            Log(L"R17.2: invalid DOS header; hook disabled.");
            return false;
        }
        ...
        if (nt->Signature != IMAGE_NT_SIGNATURE ||
            ...
            Log(L"R17.2: AMS PE validation failed; hook disabled.");
            return false;
        }

        const DWORD image_size = nt->OptionalHeader.SizeOfImage;
        if (kBuildCarRVA + 5u > image_size ||
            kGetSelectedCarIdRVA + 1u > image_size) {
            Log(L"R17.2: expected RVAs are outside AMS image; hook disabled.");
            return false;
        }

        bool build_rva_is_exec = false;
        IMAGE_SECTION_HEADER* sec = IMAGE_FIRST_SECTION(nt);
        for (WORD i = 0; i < nt->FileHeader.NumberOfSections; ++i, ++sec) {
            const DWORD begin = sec->VirtualAddress;
            const DWORD span = sec->Misc.VirtualSize > sec->SizeOfRawData
                ? sec->Misc.VirtualSize : sec->SizeOfRawData;
            const DWORD finish = begin + span;

            if (kBuildCarRVA >= begin && kBuildCarRVA < finish) {
                build_rva_is_exec =
        ...
        if (!build_rva_is_exec) {
            Log(L"R17.2: BuildCar RVA is not in an executable section.");
            return false;
        }

        BYTE* target = base + kBuildCarRVA;
        MEMORY_BASIC_INFORMATION mbi{};
        if (!VirtualQuery(target, &mbi, sizeof(mbi)) ||
            mbi.State != MEM_COMMIT) {
            Log(L"R17.2: BuildCar target is not committed memory.");
            return false;
        }

        const BYTE expected[5] = {0x55, 0x8B, 0xEC, 0x6A, 0xFF};
        if (memcmp(target, expected, sizeof(expected)) != 0) {
            Log(L"R17.2: BuildCar prologue mismatch; hook disabled.");
            return false;
        }

        g.ams = ams;
        *build_car_target = target;
        Log(L"R17.2: AMS.exe and BuildCar target validated.");
        return true;
    }
    __except(EXCEPTION_EXECUTE_HANDLER) {
        Log(L"R17.2: exception while validating AMS image; hook disabled.");
        return false;
    }
}
...
bool InstallBuildCarHook() {
    BYTE* target = nullptr;
    if (!ValidateAmsModule(&target) || !target) {
        return false;
    }
    ...
    __try {
        ...
        if (!VirtualProtect(target, 5, PAGE_EXECUTE_READWRITE, &oldProtect)) {
            Log(L"R17.2: VirtualProtect failed; hook disabled.");
            return false;
        }
        ...
        if (rel < INT32_MIN || rel > INT32_MAX) {
            ...
            Log(L"R17.2: detour displacement outside rel32 range.");
            return false;
        }
        ...
        *reinterpret_cast<std::int32_t*>(target + 1) =
        ...
        Log(L"R17.2: GS_Garage::BuildCar hook installed.");
        return true;
    }
    __except(EXCEPTION_EXECUTE_HANDLER) {
        Log(L"R17.2: exception while installing BuildCar hook.");
        return false;
    }
}
...
DWORD WINAPI RuntimeThread(LPVOID) {
    ...
    // R17.2 starts only after InitBridgeClass is invoked.
    // Give the game an additional margin before inspecting AMS code.
    Sleep(2000);
    ...
    if (kEnableOverlayUi) {
        ...
        Log(L"R17.2 delayed core active; overlay intentionally disabled for stability test.");
    }
...
extern "C" void __cdecl bridge_init(void* /*panel*/) {
    rextreme::local::Start(g.module);
}
...
BOOL WINAPI DllMain(HINSTANCE module, DWORD reason, LPVOID) {
    if (reason == DLL_PROCESS_ATTACH) {
        // Passive only. Never start the runtime from the loader entry.
        g.module = module;
